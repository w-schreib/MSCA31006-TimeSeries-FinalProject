...
- Train : 1995-02-07 - 2019-05-12
- Test  : 2019-05-13 - 2020-05-13 (1 year)

```{r}
# create test/train splits
dfTrain <- df %>% dplyr::filter(Date <= "2019-05-12")
dfTest <- df %>% dplyr::filter(Date >= "2019-05-13")
```
...
# plot time series
ts.plot(tsTrain$Chicago, main="Daily Avg Temperature in Chicago",
        ylab="Daily Avg Temperature", xlab="Date")
# acf
forecast::Acf(tsTrain$Chicago, main="ACF", lag.max = 100)
# pacf
forecast::Pacf(tsTrain$Chicago, main="PACF", lag.max = 30)
```

```{r}
# ACF of the first difference
forecast::Acf(diff(tsTrain$Chicago), main="ACF lag 1", lag.max = 100)
```

```{r}
par(mfrow=c(2,1))
# plot time series
ts.plot(tsTrain$Chicago^2, 
        main="Square of the Daily Avg Temperature in Chicago",
        ylab="Daily Avg Temperature", xlab="Date")
# acf
forecast::Acf(tsTrain$Chicago^2, main="ACF of ts^2", lag.max = 100)
```
